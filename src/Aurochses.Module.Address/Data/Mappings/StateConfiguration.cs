using Aurochses.Data.EntityFrameworkCore;
using Aurochses.Data.Extensions.MsSql;
using Aurochses.Module.Address.Data.Contract;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace Aurochses.Module.Address.Data.Mappings
{
    /// <summary>
    /// Class StateConfiguration.
    /// </summary>
    /// <seealso cref="Aurochses.Data.EntityFrameworkCore.EntityTypeConfiguration{StateEntity, Int32}" />
    public class StateConfiguration : EntityTypeConfiguration<StateEntity, int>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="StateConfiguration"/> class.
        /// </summary>
        /// <param name="schemaName">Name of the schema.</param>
        public StateConfiguration(string schemaName)
            : base(schemaName)
        {

        }

        /// <summary>
        /// Maps the specified entity type builder.
        /// </summary>
        /// <param name="entityTypeBuilder">The entity type builder.</param>
        public override void Map(EntityTypeBuilder<StateEntity> entityTypeBuilder)
        {
            // Table
            entityTypeBuilder.ToTable("State", SchemaName);

            // Primary Key
            entityTypeBuilder.HasKey(x => x.Id);

            // Properties
            entityTypeBuilder.Property(x => x.CountryId);
            entityTypeBuilder.Property(x => x.DisplayName).HasColumnType(ColumnTypes.GetNVarCharWithSpecifiedLength()).IsRequired();

            // Relationships
            entityTypeBuilder.HasOne(x => x.Country)
                .WithMany()
                .HasForeignKey(x => x.CountryId);
        }
    }
}